...
const getblockuser = require("../../models/kickmember/getblockuser");
const { getbot } = require("../../models/bot/botupdate");

let bot;

(async () => {
  const botdb = await getbot();
  const token = botdb?.data?.api;
...
setTimeout(() => {
  bot.onText(/\/start/, async (msg) => {
    ...
    if ((await getblockuser(chatId)).data) {
    } else {
  ...
  bot.on("callback_query", async (callbackQuery) => {
    ...
    if ((await getblockuser(chatId)).data) {
      bot.sendMessage(chatId, `You're blocked by admin.`);
    } else {
      switch (data) {
        case "get_weather":
          await botweather(chatId, bot, callbackQuery);
          break;
        case "get_time":
          await bottime(chatId, bot, callbackQuery);
          break;
        case "get_info":
